...
describe("ROOT billing operations page", () => {
 ...
 it("renders real billing sections and states", async () => { setup(); expect(await screen.findByRole("heading", { name: "Operações de billing" })).toBeVisible(); expect(screen.getByText(/Inadimplência, cupons, ledger e sinais antifraude/)).toBeVisible(); expect(screen.getByText("dunning")).toBeVisible(); expect(screen.getByText("ledger")).toBeVisible(); expect(screen.getByText("fraud")).toBeVisible(); expect(screen.getByText("reconciliation")).toBeVisible(); await waitFor(() => expect(screen.getByText("FAILED")).toBeVisible()); expect(screen.queryByRole("button", { name: /reembolso remoto|refund remoto/i })).not.toBeInTheDocument(); });
