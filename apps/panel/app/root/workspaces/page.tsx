...
export default function RootWorkspacesPage() {
  ...
  return <Shell>
    ...
    {error ? <section className="mb-4 flex flex-wrap items-center justify-between gap-3 border-y border-[var(--warning-border)] bg-[var(--warning-subtle)] px-4 py-4" role="alert"><p className="error">{error.message}</p><button type="button" className="btn" onClick={() => void mutate()}>Tentar novamente</button></section> : null}
        ...
        <div className="admin-form">
          ...
          {templateTenantId ? <div className="border-y border-[var(--border)] py-3" aria-live="polite">
            ...
            {previewLoading ? <div className="mt-2 grid gap-2" aria-busy="true"><span className="skeleton h-7" /><span className="skeleton h-7" /></div>
              : previewError ? <div className="mt-2 flex items-center justify-between gap-2 text-xs text-[var(--warning)]" role="alert"><span>Não foi possível carregar a prévia.</span><button type="button" className="btn" onClick={() => void retryPreview()}>Tentar novamente</button></div>
        ...
        <div className="admin-form">
          ...
          <section className="border-t border-[var(--border)] pt-5" aria-labelledby="workspace-capabilities-title">
            ...
            {loadingCapabilities ? <div className="grid gap-2" aria-busy="true">{[1, 2, 3].map((item) => <span key={item} className="skeleton h-16" />)}</div>
              : capabilitiesError ? <div className="flex items-center justify-between gap-2 border-y border-[var(--warning-border)] py-3 text-xs text-[var(--warning)]" role="alert"><span>{capabilitiesError}</span><button type="button" className="btn" onClick={() => void openEditor(editing)}>Tentar novamente</button></div>
                : <div className="divide-y divide-[var(--border)] border-y border-[var(--border)]">{sortedCapabilities.map((capability) => <div key={capability.key} className="grid gap-3 py-4 sm:grid-cols-[minmax(0,1fr)_170px] sm:items-center">
                  <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm">{capability.displayName}</strong><span className={`admin-badge${capability.enabled ? " admin-badge--ok" : ""}`}>{!capability.supported ? "Não provisionada" : capability.enabled ? "Ativo" : "Desativado"}</span><span className="admin-badge">{capabilitySourceLabel[capability.source] ?? capability.source}</span></div><p className="sub mt-1">{capability.description}</p>{capability.blockedBy.length ? <p className="mt-1 text-xs text-[var(--warning)]">Bloqueado por {capability.blockedBy.map((key) => capabilityName(capabilities, key)).join(", ")}.</p> : null}{capability.dependencies.length ? <p className="mono mt-1 type-caption text-[var(--text-muted)]">Depende de {capability.dependencies.map((key) => capabilityName(capabilities, key)).join(", ")}</p> : null}</div>
          ...
          {pending ? <section className="border-y border-[var(--warning-border)] bg-[var(--warning-subtle)] px-4 py-4" role="alertdialog" aria-labelledby="cascade-confirmation-title"><div className="flex gap-3"><WarningCircle className="shrink-0 text-[var(--warning)]" size={20} aria-hidden="true" /><div><strong id="cascade-confirmation-title" className="text-sm">Confirmar alteração em cascata?</strong><p className="sub mt-1">Esta mudança também afeta: {pending.affected.map((key) => capabilityName(capabilities, key)).join(", ")}.</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" className="btn active:scale-[.98]" disabled={savingCapability !== null} onClick={() => void applyCapability(pending, true)}>{savingCapability ? "Aplicando…" : "Confirmar mudança"}</button><button type="button" className="btn" disabled={savingCapability !== null} onClick={() => setPending(null)}>Cancelar</button></div></div></div></section> : null}
