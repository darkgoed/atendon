...
import { Buildings, DoorOpen, PencilSimple, Plus, WarningCircle } from "@phosphor-icons/react";
...
export default function RootWorkspacesPage() {
  ...
  return <Shell>
    <header className="pagehead" style={{ "--eyebrow": '"PAINEL · ROOT"' } as React.CSSProperties}><div><h1>Workspaces</h1><p>Provisionamento, capabilities comerciais e acesso assistido ROOT.</p></div></header>
    {message ? <p className="error mb-4" role="alert">{message}</p> : null}
    {notice ? <p className="accent mb-4 text-sm" role="status">{notice}</p> : null}
    {error ? <section className="mb-4 flex flex-wrap items-center justify-between gap-3 border-y border-[var(--warn-border)] bg-[var(--warn-bg)] px-4 py-4" role="alert"><p className="error">{error.message}</p><button type="button" className="btn warn" onClick={() => void mutate()}>Tentar novamente</button></section> : null}
    ...
    <section className={created || editing ? "admin-grid admin-grid--sidebar" : "admin-stack max-w-md"}>
      <form className="card admin-card" aria-busy={creating} onSubmit={submit}>
        ...
        <div className="admin-form">
          <label className="field"><span className="label">Nome</span><input className="input" name="name" placeholder="Operação Sul" disabled={creating} required /></label>
          ...
          {templateTenantId ? <div className="border-y border-[var(--border)] py-3" aria-live="polite">
            ...
            {previewLoading ? <div className="mt-2 grid gap-2" aria-busy="true"><span className="skeleton h-7" /><span className="skeleton h-7" /></div>
              : previewError ? <div className="mt-2 flex items-center justify-between gap-2 text-xs text-[var(--warn)]" role="alert"><span>Não foi possível carregar a prévia.</span><button type="button" className="btn warn" onClick={() => void retryPreview()}>Tentar novamente</button></div>
        ...
        <div className="admin-form">
          ...
          <section className="border-t border-[var(--border)] pt-5" aria-labelledby="workspace-capabilities-title">
            ...
            {loadingCapabilities ? <div className="grid gap-2" aria-busy="true">{[1, 2, 3].map((item) => <span key={item} className="skeleton h-16" />)}</div>
              : capabilitiesError ? <div className="flex items-center justify-between gap-2 border-y border-[var(--warn-border)] py-3 text-xs text-[var(--warn)]" role="alert"><span>{capabilitiesError}</span><button type="button" className="btn warn" onClick={() => void openEditor(editing)}>Tentar novamente</button></div>
                : <div className="divide-y divide-[var(--border)] border-y border-[var(--border)]">{sortedCapabilities.map((capability) => <div key={capability.key} className="grid gap-3 py-4 sm:grid-cols-[minmax(0,1fr)_170px] sm:items-center">
                  <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm">{capability.displayName}</strong><span className={`admin-badge${capability.enabled ? " admin-badge--ok" : ""}`}>{!capability.supported ? "Não provisionada" : capability.enabled ? "Ativo" : "Desativado"}</span><span className="admin-badge">{capabilitySourceLabel[capability.source] ?? capability.source}</span></div><p className="sub mt-1">{capability.description}</p>{capability.blockedBy.length ? <p className="mt-1 text-xs text-[var(--warn)]">Bloqueado por {capability.blockedBy.map((key) => capabilityName(capabilities, key)).join(", ")}.</p> : null}{capability.dependencies.length ? <p className="mono mt-1 text-[10px] text-[var(--faint)]">Depende de {capability.dependencies.map((key) => capabilityName(capabilities, key)).join(", ")}</p> : null}</div>
    ...
    <section className="card admin-card"><div className="cardtitle"><span>Catálogo de workspaces</span><span className="sub">{workspaces.length} registro(s)</span></div>
