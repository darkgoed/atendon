  ...
  return (
    <>
      <div className="agenda-detail-actions flex flex-wrap gap-2" role="group" aria-label={`Ações para ${selectedAppointment.lead_nome ?? selectedAppointment.lead_telefone}`}>
        {permissions.canReply ? <Tooltip content={(openingConversationId === selectedAppointment.id ? "Iniciando conversa…" : selectedAppointment.conversation_id ? "Abrir conversa" : "Iniciar conversa")}><button type="button" className="btn icon-button active:scale-[.98]" disabled={openingConversationId === selectedAppointment.id} onClick={() => void openConversation(selectedAppointment)}><WhatsappLogo size={15} aria-hidden="true" /><span className="sr-only">{(openingConversationId === selectedAppointment.id ? "Iniciando conversa…" : selectedAppointment.conversation_id ? "Abrir conversa" : "Iniciar conversa")}</span></button></Tooltip> : null}
        {selectedAppointment.meet_link ? (
          <Tooltip content={(joiningAppointmentId === selectedAppointment.id ? "Verificando reunião…" : joinBlockedIds.has(selectedAppointment.id) ? "Sala indisponível" : joinReadyUrl ? "Abrir Meet liberado" : "Entrar no Meet")}><button type="button" className="btn icon-button active:scale-[.98]" disabled={joiningAppointmentId === selectedAppointment.id || joinBlockedIds.has(selectedAppointment.id)} onClick={() => { if (joinReadyUrl) window.open(joinReadyUrl, "_blank", "noopener,noreferrer"); else void joinAppointment(selectedAppointment); }}>
            <VideoCamera size={15} aria-hidden="true" /><span className="sr-only">{(joiningAppointmentId === selectedAppointment.id ? "Verificando reunião…" : joinBlockedIds.has(selectedAppointment.id) ? "Sala indisponível" : joinReadyUrl ? "Abrir Meet liberado" : "Entrar no Meet")}</span>
          </button></Tooltip>
        ) : null}
        {(isActiveAppointment(selectedAppointment.status) || selectedAppointment.status === "concluido" || selectedAppointment.status === "no_show") && permissions.canReschedule ? <Tooltip content="Reagendar"><button type="button" className="btn active:scale-[.98] icon-button" onClick={() => beginReschedule(selectedAppointment)}><CalendarDots size={15} aria-hidden="true" /><span className="sr-only">Reagendar</span></button></Tooltip> : null}
        ...
        {isActiveAppointment(selectedAppointment.status) && permissions.canNoShow ? <Tooltip content="Não compareceu"><button type="button" className="btn warn active:scale-[.98] icon-button" onClick={() => beginFinalAction(selectedAppointment, "no_show")}><UserMinus size={15} aria-hidden="true" /><span className="sr-only">Não compareceu</span></button></Tooltip> : null}
        {selectedAppointment.status === "concluido" && permissions.canNoShow ? <Tooltip content="Corrigir para não compareceu"><button type="button" className="btn warn active:scale-[.98] icon-button" onClick={() => beginFinalAction(selectedAppointment, "no_show")}><UserMinus size={15} aria-hidden="true" /><span className="sr-only">Corrigir para não compareceu</span></button></Tooltip> : null}
        ...
        {isActiveAppointment(selectedAppointment.status) && permissions.canCancel ? <Tooltip content="Cancelar"><button type="button" className="btn warn active:scale-[.98] icon-button" onClick={() => beginFinalAction(selectedAppointment, "cancel")}><X size={15} aria-hidden="true" /><span className="sr-only">Cancelar</span></button></Tooltip> : null}
        {permissions.canDeleteLead ? <Tooltip content="Excluir agendamento"><button type="button" className="btn warn active:scale-[.98] icon-button" aria-label={`Excluir agendamento de ${selectedAppointment.lead_nome ?? selectedAppointment.lead_telefone}`} onClick={() => setConfirmDeleteLead(true)}><Trash size={15} aria-hidden="true" /><span className="sr-only">Excluir agendamento</span></button></Tooltip> : null}
      </div>
            ...
            return (
              <article key={recording.id} className="grid gap-3 border-t border-[var(--border)] pt-4 first:border-t-0 first:pt-0">
                <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
                  ...
                  {playable ? <Tooltip content="Baixar gravação"><a className="btn icon-button icon-button--sm active:translate-y-px" href={fileUrl} download={recording.file_name || undefined}><DownloadSimple size={14} aria-hidden="true" /><span className="sr-only">Baixar</span></a></Tooltip> : <span className="type-caption text-[var(--warning-text)]">{recording.status}</span>}
                </div>
