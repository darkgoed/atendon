  ...
  return (
    <ModalDialog className="agenda-detail-dialog" labelledBy="agenda-detail-title" describedBy="agenda-detail-description" onClose={closeAppointment}>
      ...
      <div className="agenda-detail-summary">
        <div><span className="label">Status</span><strong>{isAppointmentResultPending(selectedAppointment, now) ? "Resultado pendente" : APPOINTMENT_STATUS_LABELS[selectedAppointment.status]}</strong>{isAppointmentResultPending(selectedAppointment, now) ? <small className="mt-1 block text-[var(--warning)]">O horário terminou sem um desfecho registrado.</small> : null}</div>
        <div>
          ...
          {detailAssigneesError ? <small className="mt-1 block text-[var(--warning)]">Não foi possível carregar os responsáveis.</small> : null}
        </div>
      </div>
...
function NoShowConfirmation({ actions }: { actions: AgendaActions }) {
  ...
  return (
    <section className="grid gap-4 border-y border-[var(--warning-border)] bg-[var(--warning-subtle)] px-4 py-5" aria-labelledby="agenda-final-action-title">
      <div><span className="label">Confirmar ação</span><strong id="agenda-final-action-title" className="mt-1 block text-sm">Registrar não comparecimento</strong><p className="mt-1 text-sm text-[var(--warning)]">Ao confirmar, uma ação de recuperação será criada para retomar este contato.</p></div>
      {actionError ? <p className="error" role="alert">{actionError}</p> : null}
      <div className="flex flex-wrap justify-end gap-2"><button type="button" className="btn" disabled={Boolean(pendingActionId)} onClick={() => { setFinalAction(null); setActionError(""); }}>Voltar</button><button type="button" className="btn active:scale-[.98]" disabled={Boolean(pendingActionId)} onClick={() => void runFinalAction()}>{pendingActionId ? "Salvando…" : "Marcar não comparecimento"}</button></div>
    </section>
  );
}
  ...
  return (
    <>
      <div className="agenda-detail-actions flex flex-wrap gap-2" role="group" aria-label={`Ações para ${selectedAppointment.lead_nome ?? selectedAppointment.lead_telefone}`}>
        ...
        {isActiveAppointment(selectedAppointment.status) && permissions.canNoShow ? <button type="button" className="btn active:scale-[.98]" onClick={() => beginFinalAction(selectedAppointment, "no_show")}><UserMinus size={15} aria-hidden="true" /> Não compareceu</button> : null}
        {selectedAppointment.status === "concluido" && permissions.canNoShow ? <button type="button" className="btn active:scale-[.98]" onClick={() => beginFinalAction(selectedAppointment, "no_show")}><UserMinus size={15} aria-hidden="true" /> Corrigir para não compareceu</button> : null}
        ...
        {isActiveAppointment(selectedAppointment.status) && permissions.canCancel ? <button type="button" className="btn active:scale-[.98]" onClick={() => beginFinalAction(selectedAppointment, "cancel")}><X size={15} aria-hidden="true" /> Cancelar</button> : null}
        {permissions.canDeleteLead ? <button type="button" className="btn active:scale-[.98]" aria-label={`Excluir agendamento de ${selectedAppointment.lead_nome ?? selectedAppointment.lead_telefone}`} title="Excluir agendamento" onClick={() => setConfirmDeleteLead(true)}><Trash size={15} aria-hidden="true" /> Excluir agendamento</button> : null}
      </div>
    ...
    <section className="border-t border-[var(--border)] pt-5" aria-labelledby={`appointment-recordings-${appointmentId}`}>
      ...
      {isLoading ? (
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-y border-[var(--warning-border)] bg-[var(--warning-subtle)] px-3 py-4" role="alert">
          <p className="m-0 text-xs text-[var(--warning)]">{error instanceof Error ? error.message : "Não foi possível carregar as gravações."}</p>
          <button type="button" className="btn px-2 py-1 text-xs active:translate-y-px inline-flex items-center gap-1.5" onClick={() => void mutate()}><ArrowClockwise size={14} aria-hidden="true" />Tentar novamente</button>
        </div>
      ) : recordings.length === 0 ? (
              ...
              <article key={recording.id} className="grid gap-3 border-t border-[var(--border)] pt-4 first:border-t-0 first:pt-0">
                <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
                  <div className="grid min-w-0 grid-cols-[30px_minmax(0,1fr)] items-center gap-3">
                    <span className="grid size-8 place-items-center rounded-full border border-[var(--border)] text-[var(--primary)]"><FileVideo size={16} aria-hidden="true" /></span>
                  ...
                  {playable ? <a className="btn px-2 py-1 text-xs active:translate-y-px" href={fileUrl} download={recording.file_name || undefined}><DownloadSimple size={14} aria-hidden="true" />Baixar</a> : <span className="mono type-caption uppercase text-[var(--warning)]">{recording.status}</span>}
                </div>
