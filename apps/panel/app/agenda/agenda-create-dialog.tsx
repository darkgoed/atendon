      ...
      <form className="grid gap-5" onSubmit={createAppointment}>
        {permissions.canCreateLeads ? (
          <div className="grid grid-cols-2 gap-1 rounded-lg border border-[var(--border)] p-1" role="group" aria-label="Origem do contato">
            <button type="button" className={`rounded px-3 py-2 text-xs font-semibold active:scale-[.98] ${createMode === "existing" ? "bg-[var(--primary-subtle)] text-[var(--primary)]" : "text-[var(--text-secondary)]"}`} aria-pressed={createMode === "existing"} onClick={() => setCreateMode("existing")}>Lead existente</button>
            <button type="button" className={`rounded px-3 py-2 text-xs font-semibold active:scale-[.98] ${createMode === "new" ? "bg-[var(--primary-subtle)] text-[var(--primary)]" : "text-[var(--text-secondary)]"}`} aria-pressed={createMode === "new"} onClick={() => { setCreateMode("new"); requestAnimationFrame(() => newContactNameRef.current?.focus()); }}>Novo contato</button>
          </div>
        ) : null}
        {createMode === "existing" ? (
        ...
        <section className="grid gap-3 border-t border-[var(--border)] pt-5" aria-labelledby="agenda-create-assignee-title">
          <div className="flex items-start gap-3"><UsersThree className="mt-0.5 text-[var(--primary)]" size={19} aria-hidden="true" /><div><strong id="agenda-create-assignee-title" className="block text-sm">Closer responsável</strong><p className="sub mt-1 text-xs">A lista considera disponibilidade e conflitos no horário escolhido.</p></div></div>
          {createAssigneesLoading ? <div className="skeleton h-11" aria-label="Carregando closers" /> : createAssigneesError ? (
            <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-[var(--warning)]" role="alert"><span>{messageFrom(createAssigneesError, "Não foi possível carregar os closers.")}</span><button type="button" className="btn" onClick={() => void retryCreateAssignees()}>Tentar novamente</button></div>
