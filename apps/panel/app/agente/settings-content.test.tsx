...
import { AgentSettingsContent } from "./settings-content";

describe("Conteúdo reutilizável do agente (aninhamento /configuracoes/agente)", () => {
  afterEach(cleanup);

  it("Content renderiza o cabeçalho sem Shell", () => {
    render(<AgentSettingsContent />);
    expect(screen.getByRole("heading", { name: "Agente principal" })).toBeVisible();
  ...
  it("página legada envolve o mesmo conteúdo em Shell", () => {
    ...
    expect(within(shell).getByRole("heading", { name: "Agente principal" })).toBeVisible();
  });
});
