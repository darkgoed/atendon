...
function payload(path, method, useRoot, route) {
  ...
  if (EXPECTED_PROVIDER_FAILURES.has(path)) return { status: 503, body: { error: "provider-unavailable", message: "Provedor de videochamada indisponível no ambiente de QA." } };
...
async function boundedEntityReady(page, contract) {
  if (contract?.state !== "populated" || !contract.entitySelector || !contract.marker) return;
...
async function measure(page, requestedTheme, contract) {
  const measured = await page.evaluate(async ({ requested, marker }) => {
    ...
    const entityNodes = marker.state === "populated" ? [...document.querySelectorAll(marker.entitySelector)].filter(visible) : [];
    ...
    return { heading: { matched: Boolean(heading), text: heading?.textContent?.trim() ?? null, count: headings.length }, entity: { matched: marker.state !== "populated" || entity }, textLength: text.trim().length, loading: Boolean(document.querySelector("[aria-busy=true], .loading-state, [data-loading=true], [role=status][aria-label*='carreg' i]")), theme: { dataset: document.documentElement.dataset.theme ?? null, background: rootStyle.getPropertyValue("--bg").trim() || bodyStyle.backgroundColor, requested }, errorText: /application error|internal server error|fixture-gap/i.test(text) ? text.match(/.{0,40}(application error|internal server error|fixture-gap).{0,80}/i)?.[0] ?? null : null, viewport: { width: innerWidth, height: innerHeight }, controls, rootGeometry, metrics: { clientWidth: document.documentElement.clientWidth, scrollWidth: document.documentElement.scrollWidth, horizontalOverflow: document.documentElement.scrollWidth > document.documentElement.clientWidth + 1, clippedButtons: clippedButtonDetails.length > 0, clippedButtonDetails } };
...
async function main() {
  ...
  const selected = inventory.filter(({ route }) => !routeFilter || new RegExp(routeFilter, "i").test(route));
  ...
  try {
    for (const entry of selected) for (const theme of themes) for (const viewport of viewports) {
      const route = entry.route, requested = routeReplacements[route] ?? route, contract = domainContracts[route] ?? contractFor(route);
      ...
      page.on("console", (msg) => { if (msg.type() === "error" && !(EXPECTED_PROVIDER_FAILURES.has(route === "/meet/[roomId]" ? "/meet/rooms/qa-room/token" : route === "/reuniao/[code]" ? "/meet/join/qa-code" : "") && /503 \(Service Unavailable\)/.test(msg.text()))) consoleErrors.push(msg.text()); }); page.on("pageerror", (error) => pageErrors.push(String(error)));
      page.on("requestfailed", (req) => { if (!benignExternal(req.url())) failedRequests.push(`${req.method()} ${req.url()} ${req.failure()?.errorText ?? "failed"}`); });
      const key = `${route}|${theme}|${viewport.name}`; if (seen.has(key)) throw new Error(`duplicate audit record key: ${key}`); seen.add(key);
      const record = { version: "design-audit-v2", baseline: process.env.AUDIT_PHASE !== "final", buildMarker: process.env.AUDIT_BUILD_MARKER ?? "baseline3499", route, requested, theme: { requested: theme }, viewport, status: "failed", finalUrl: null, httpStatus: null, screenshot: null, assets, gaps, consoleErrors, pageErrors, failedRequests, axe: null, metrics: null, heading: null, entity: null, loading: false, errorText: null, reason: null };
      try {
        ...
        const validation = validateRecord(record, contract); record.status = validation.valid ? "passed" : (gaps.length ? "fixture-gap" : "failed"); record.reason = validation.errors.join("; ");
        record.screenshot = resolve(output, "screenshots", `${route.replaceAll("/", "_").replaceAll("[", "").replaceAll("]", "") || "home"}--${theme}--${viewport.name}.png`); await page.screenshot({ path: record.screenshot, fullPage: true, animations: "disabled" }); await writeFile(resolve(output, "dom", `${(records.length + 1).toString().padStart(4, "0")}.html`), await page.content());
  ...
  console.log(JSON.stringify({ output, routes: selected.length, records: records.length, counts: report.counts, unknownApiRequests: report.unknownApiRequests }, null, 2));
  if (records.length !== report.inventory.expectedRecords || report.unknownApiRequests.length || records.some((record) => record.status !== "passed")) process.exitCode = 2;
}
