    ...
    >
      {() => (
        <>
          ...
          {!isLoading && !error && !data?.tags.length ? <p className="p-2 text-xs type-secondary">Nenhuma etiqueta disponível.</p> : null}
          {data?.tags.filter((tag) => !tag.archived_at).map((tag) => {
            ...
            return (
              <button
                key={tag.id}
                type="button"
                className="flex min-h-9 items-center gap-2 rounded px-2 text-left text-xs lead-tag-picker__option transition-token is-pressable"
