...
export function TagCatalogSettings() {
  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-30 grid place-items-end overlay-backdrop sm:place-items-center" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setOpen(false); }}>
      <section className="max-h-[88dvh] tag-dialog-width w-full overflow-y-auto rounded-t-2xl border border-semantic surface-elevated p-4 sm:rounded-xl sm:p-5" role="dialog" aria-modal="true" aria-labelledby="tag-catalog-title">
        <header className="mb-5 flex items-start justify-between gap-4"><div><h2 id="tag-catalog-title" className="text-lg font-semibold">Catálogo de etiquetas</h2><p className="mt-1 text-xs type-secondary">O mesmo catálogo é usado em Conversas, Leads e Pipeline.</p></div><button type="button" className="grid size-9 place-items-center rounded border border-semantic is-pressable" onClick={() => setOpen(false)} aria-label="Fechar catálogo"><X size={16} /></button></header>
        ...
        {!isLoading && !error && !data?.tags.length ? <p className="py-5 text-center text-sm type-secondary">Crie a primeira etiqueta para organizar os leads.</p> : null}
...
function TagEditor({ tag, onChanged }: { tag: CatalogTag; onChanged: () => unknown | Promise<unknown> }) {
  ...
  return <div className={`grid gap-2 rounded border border-semantic p-3 ${tag.archived_at ? "opacity-55" : ""}`}><div className="grid grid-cols-[minmax(0,1fr)_64px_auto] items-end gap-2"><label className="field"><span className="label">Nome</span><input className="input" value={name} onChange={(event) => setName(event.target.value)} disabled={Boolean(tag.archived_at)} /></label><label className="field"><span className="label">Cor</span><input className="input h-10 p-1" type="color" value={color} onChange={(event) => setColor(event.target.value.toUpperCase())} disabled={Boolean(tag.archived_at)} /></label>{tag.archived_at ? <span className="pb-2 type-caption uppercase type-muted">Arquivada</span> : <button type="button" className="btn" onClick={() => void patch({ name: name.trim(), color })} disabled={pending || !name.trim()}>Salvar</button>}</div>{!tag.archived_at ? <button type="button" className="justify-self-start type-label type-warning hover:underline is-pressable" onClick={() => { if (window.confirm(`Arquivar a etiqueta “${tag.name}”?`)) void patch({ archived: true }); }} disabled={pending}><Archive size={13} className="mr-1 inline" aria-hidden="true" />Arquivar</button> : null}{error ? <p className="error" role="alert">{error}</p> : null}</div>;
}
