  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-30 grid place-items-end bg-[color-mix(in_srgb,var(--bg)_68%,transparent)] sm:place-items-center" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setOpen(false); }}>
      <section className="max-h-[88dvh] tag-dialog-width w-full overflow-y-auto rounded-t-2xl border border-[var(--border)] bg-[var(--surface-elevated)] p-4 sm:rounded-xl sm:p-5" role="dialog" aria-modal="true" aria-labelledby="tag-catalog-title">
        ...
        {!isLoading && !error && !data?.tags.length ? <p className="py-5 text-center text-sm text-[var(--text-secondary)]">Crie a primeira etiqueta para organizar os leads.</p> : null}
        <section className={styles.dialogSection}><h3 className="text-sm font-semibold">Nova etiqueta</h3><div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_5rem]"><Field label="Nome"><Input value={name} onChange={(event) => setName(event.target.value)} maxLength={80} /></Field><Field label="Cor"><Input className="h-10 p-1" type="color" value={color} onChange={(event) => setColor(event.target.value.toUpperCase())} /></Field></div><SaveButton state={pending ? "busy" : save.state} className="mt-3" icon={<Plus size={15} aria-hidden="true" />} onClick={() => void create()} disabled={!name.trim()}>Criar etiqueta</SaveButton>{actionError ? <p className="error mt-2" role="alert">{actionError}</p> : null}</section>
      </section>
    </div> : null}
    <SaveToast show={save.done}>Etiqueta criada</SaveToast>
  </>;
}
...
function TagEditor({ tag, onChanged }: { tag: CatalogTag; onChanged: () => unknown | Promise<unknown> }) {
  ...
  async function patch(body: Record<string, unknown>) {
    ...
    try { await api(`/organization/tags/${tag.id}`, { method: "PATCH", body: JSON.stringify(body) }); await onChanged(); save.markDone(); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Falha ao atualizar etiqueta"); }
    finally { setPending(false); }
  }
  return <div className={`grid gap-2 rounded border border-[var(--border)] p-3 ${tag.archived_at ? "opacity-55" : ""}`}><div className="grid grid-cols-[minmax(0,1fr)_64px_auto] items-end gap-2"><label className="field"><span className="label">Nome</span><input className="input" value={name} onChange={(event) => setName(event.target.value)} disabled={Boolean(tag.archived_at)} /></label><label className="field"><span className="label">Cor</span><input className="input h-10 p-1" type="color" value={color} onChange={(event) => setColor(event.target.value.toUpperCase())} disabled={Boolean(tag.archived_at)} /></label>{tag.archived_at ? <span className="pb-2 type-caption uppercase text-[var(--text-muted)]">Arquivada</span> : <SaveButton state={pending ? "busy" : save.state} onClick={() => void patch({ name: name.trim(), color })} disabled={!name.trim()}>Salvar</SaveButton>}</div>{!tag.archived_at ? <IconButton label="Arquivar" size="sm" className="justify-self-start text-[var(--warning-text)]" onClick={() => { if (window.confirm(`Arquivar a etiqueta “${tag.name}”?`)) void patch({ archived: true }); }} disabled={pending}><Archive size={13} aria-hidden="true" /></IconButton> : null}{error ? <p className="error" role="alert">{error}</p> : null}<SaveToast show={save.done}>Etiqueta salva</SaveToast></div>;
}
