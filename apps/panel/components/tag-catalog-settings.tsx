...
export function TagCatalogSettings() {
  ...
  return <>
    <button type="button" className="btn" onClick={() => setOpen(true)}><Tag size={15} aria-hidden="true" />Catálogo</button>
    {open ? <div className="fixed inset-0 z-30 grid place-items-end bg-[color-mix(in_srgb,var(--app)_68%,transparent)] sm:place-items-center" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setOpen(false); }}>
      <section className="max-h-[88dvh] w-full overflow-y-auto rounded-t-2xl border border-[var(--border)] bg-[var(--dialog)] p-4 sm:w-[min(580px,calc(100vw-32px))] sm:rounded-xl sm:p-5" role="dialog" aria-modal="true" aria-labelledby="tag-catalog-title">
        ...
        <section className="mt-6 border-t border-[var(--border)] pt-5"><h3 className="text-sm font-semibold">Nova etiqueta</h3><div className="mt-3 grid grid-cols-[minmax(0,1fr)_72px] gap-2"><label className="field"><span className="label">Nome</span><input className="input" value={name} onChange={(event) => setName(event.target.value)} maxLength={80} /></label><label className="field"><span className="label">Cor</span><input className="input h-10 p-1" type="color" value={color} onChange={(event) => setColor(event.target.value.toUpperCase())} /></label></div><button type="button" className="btn primary mt-3" onClick={() => void create()} disabled={!name.trim() || pending}><Plus size={15} aria-hidden="true" />{pending ? "Criando…" : "Criar etiqueta"}</button>{actionError ? <p className="error mt-2" role="alert">{actionError}</p> : null}</section>
      </section>
    </div> : null}
  </>;
}
