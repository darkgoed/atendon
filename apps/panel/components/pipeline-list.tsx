...
export function PipelineList({ leads, stages, members, legacy, loading, canMove, canSelect, selectedIds, pendingLeadIds, onToggleSelected, onMoveRequest }: {
  ...
  return <div className="admin-table-wrap responsive-table-wrap pipeline-list">
    <table className="admin-table responsive-table">
      ...
      <tbody>{leads.map((lead) => {
        ...
        return <tr key={lead.id}>
          ...
          {canMove ? <td data-label="Ações"><button type="button" className="btn min-h-8 px-2 py-1 text-[11px]" disabled={pendingLeadIds.has(lead.id)} onClick={() => onMoveRequest(lead)}>Mover</button></td> : null}
        </tr>;
      })}</tbody>
    </table>
  </div>;
