...
export function RateRing({ value, tone }: { value: number; tone: string }) {
  ...
  return (
    <svg viewBox="0 0 64 64" className="size-16 flex-none" role="img" aria-label={`${Math.round(clamped)}%`}>
      <circle cx="32" cy="32" r={radius} fill="none" stroke="var(--surface-active)" strokeWidth="5" />
      <circle
        cx="32"
        cy="32"
        r={radius}
        ...
        {[0, 0.5, 1].map((ratio) => {
          ...
          return (
            <g key={ratio}>
              ...
              <text x={left - 9} y={tickY + 4} textAnchor="end" fill="var(--text-muted)" fontSize="10">{value}</text>
            </g>
          );
        })}
        {data.map((item, index) => index % tickEvery === 0 || index === data.length - 1 ? (
          <text key={item.day} x={x(index)} y={height - 10} textAnchor="middle" fill="var(--text-muted)" fontSize="10">
            {new Intl.DateTimeFormat("pt-BR", { day: "2-digit", month: "2-digit", timeZone: "UTC" }).format(new Date(`${item.day}T12:00:00.000Z`))}
          </text>
        ) : null)}
        <polyline fill="none" stroke="var(--primary)" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" points={points("scheduled")} />
        <polyline fill="none" stroke="var(--success)" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" strokeDasharray="7 5" points={points("completed")} />
        <polyline fill="none" stroke="var(--warning)" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" strokeDasharray="2 4" points={points("no_show")} />
        {data.map((item, index) => (
          <g key={`${item.day}-points`}>
            <circle cx={x(index)} cy={y(item.scheduled)} r="3" fill="var(--surface)" stroke="var(--primary)" strokeWidth="2" />
            <circle cx={x(index)} cy={y(item.completed)} r="3" fill="var(--surface)" stroke="var(--success)" strokeWidth="2" />
            <circle cx={x(index)} cy={y(item.no_show)} r="3" fill="var(--surface)" stroke="var(--warning)" strokeWidth="2" />
          </g>
        ))}
      </svg>
    </div>
