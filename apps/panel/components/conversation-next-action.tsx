    ...
    <section className={`card ${due ? "border-[var(--warning-border)]" : ""}`} aria-labelledby="conversation-next-action-title">
      ...
      {open ? <ModalDialog labelledBy="next-action-dialog-title" onClose={() => { if (!busy) setOpen(false); }}>
        <form onSubmit={save} className="grid gap-4">
          ...
          <label className="field"><span className="label">Descrição <b aria-hidden="true">*</b></span><Textarea className="input min-h-24" value={action} maxLength={500} required onChange={(event) => setAction(event.target.value)} /><small className="text-[var(--text-muted)]">{action.length}/500</small></label>
