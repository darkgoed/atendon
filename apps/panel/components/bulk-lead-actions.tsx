...
export function BulkLeadActions({ selected, onClear, onChanged }: { selected: BulkItem[]; onClear: () => void; onChanged: () => unknown | Promise<unknown> }) {
  ...
  return <aside className="bulk-lead-actions" aria-label="Ações em lote">
    ...
    {preview && !preview.valid ? <div className="sm:col-span-4 rounded border border-[var(--warning-border)] bg-[var(--warning-subtle)] p-2 text-xs type-warning" role="alert"><strong>O lote inteiro foi bloqueado.</strong><ul className="mt-1 list-disc pl-4">{preview.errors.slice(0, 5).map((item) => <li key={`${item.id}:${item.code}`}>{item.message}</li>)}</ul></div> : null}
    {preview?.valid ? <div className="sm:col-span-4 rounded border border-success surface-success p-2 text-xs type-success" role="status"><strong>Prévia validada para {preview.count} lead(s).</strong> {preview.undoable ? "Esta ação poderá ser desfeita por 30 segundos." : "Esta mudança de status não poderá ser desfeita pelo atalho."}</div> : null}
