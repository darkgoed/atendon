...
export function WebPushSettings() {
  ...
  return (
    <section className="border-t border-[var(--border)] pt-5" aria-labelledby="web-push-title">
      ...
      {!supported ? <p className="mt-3 text-sm text-[var(--warning-text)]">Este navegador não oferece Web Push.</p> : null}
      {supported && data && !data.configured ? <p className="mt-3 text-sm text-[var(--warning-text)]">VAPID ainda não foi configurado pelo administrador.</p> : null}
