...
export function FailedMessageRecovery({
  ...
  return (
    <section className="card mb-6" aria-labelledby="failed-message-recovery-title">
        ...
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            ...
            {recovery.available > 0 ? (
              ...
              <p className="mt-1 text-sm ">Nenhuma mensagem recuperável está aguardando reenvio.</p>
            )}
            ...
            {!connected ? <p className="mt-2 text-xs ">Reconecte o WhatsApp antes de reenviar.</p> : null}
            {error ? <p className="error mt-2" role="alert">{error}</p> : null}
          </div>
        </div>
        {!confirming ? (
          <button type="button" className="btn" disabled={disabled} onClick={onConfirm}>
      ...
      {confirming ? (
        <div className="mt-4 border-t  pt-4" role="alertdialog" aria-label="Confirmar reenvio de mensagens">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <button type="button" className="btn" disabled={recovering || !connected} onClick={onRecover}>
              <ArrowClockwise size={16} className={recovering ? "animate-spin" : ""} aria-hidden="true" />
