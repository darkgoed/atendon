...
export function ConversationQueueManager({ onSaved }: { onSaved?: () => void | Promise<void> }): ReactElement {
  ...
  const [createDraft, setCreateDraft] = useState({ name: "", color: "#64748B" });
  const [editDraft, setEditDraft] = useState({ name: "", color: "#64748B" });
  ...
  async function create() {
    ...
    try { await api("/conversation-queues", { method: "POST", body: JSON.stringify({ name, color: createDraft.color }) }); setCreateDraft({ name: "", color: "#64748B" }); await refresh(); }
  ...
  return <section className="card" aria-labelledby="conversation-queues-title">
    <button type="button" className="flex w-full items-center justify-between gap-2 text-left" onClick={() => setCollapsed((value) => !value)} aria-expanded={!collapsed} aria-controls="conversation-queues-content"><span id="conversation-queues-title" className="cardtitle">Filas de atendimento</span><span className="text-[11px] text-[var(--faint)]">{active.length} ativas {collapsed ? "＋" : "－"}</span></button>
