...
const DEFAULT_QUEUE_COLOR = "#64748B";
...
export function ConversationQueueManager({ onSaved }: { onSaved?: () => void | Promise<void> }): ReactElement {
  ...
  return <section className="card" aria-labelledby="conversation-queues-title">
    <button type="button" className="flex w-full items-center justify-between gap-2 text-left" onClick={() => setCollapsed((value) => !value)} aria-expanded={!collapsed} aria-controls="conversation-queues-content"><span id="conversation-queues-title" className="cardtitle">Filas de atendimento</span><span className="text-xs text-[var(--faint)]">{active.length} ativas {collapsed ? "＋" : "－"}</span></button>
