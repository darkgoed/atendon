...
export function TripzAttachmentCard({
  ...
  return (
    ...
    >
      <span className="grid h-[3.25rem] place-items-center overflow-hidden border-r tripz-border-border tripz-bg-bg tripz-text-primary">
        {isImage ? (
      ...
      <span className="grid min-w-0 content-center gap-0.5 px-3 py-2">
        <strong className="truncate text-xs font-semibold tripz-text-text">{attachment.filename}</strong>
        <span className={`flex min-w-0 items-center gap-1.5 text-[10px] ${failed ? "tripz-text-warning" : "tripz-text-text_muted"}`}>
