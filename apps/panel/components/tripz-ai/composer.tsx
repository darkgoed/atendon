      ...
      <div className="mx-auto grid w-full max-w-[52rem] gap-2">
              ...
              return (
                <article key={item.localId} className="grid w-[13.5rem] shrink-0 grid-cols-[2.75rem_minmax(0,1fr)_auto] items-center gap-2 border rounded-[var(--radius-md)] border-[var(--border)] bg-[var(--surface)] p-2">
                  ...
                  <span className="grid min-w-0 gap-0.5">
                    ...
                    <span className={`truncate text-[9px] ${item.status === "error" ? "text-[var(--warning-text)]" : "text-[var(--text-muted)]"}`}>
                      {item.status === "uploading" ? "Enviando…" : item.status === "uploaded" ? "Pronto" : item.error ?? formatTripzFileSize(item.file.size)}
                    </span>
                  </span>
                  <button type="button" className="grid h-7 w-7 place-items-center bg-transparent text-[var(--text-muted)] transition-[background,transform] hover:bg-[var(--warning-subtle)] hover:text-[var(--warning-text)] active:scale-[.96]" onClick={() => removeItem(item.localId)} aria-label={`Remover ${item.file.name}`}>
        ...
        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-end gap-2 border rounded-[var(--radius-md)] border-[var(--border-strong)] bg-[var(--surface)] p-2 shadow-[inset_0_1px_0_color-mix(in_srgb,var(--text)_5%,transparent)] focus-within:border-[var(--primary)] focus-within:shadow-[var(--focus-ring)]">
          ...
          <textarea
            ref={textareaRef}
            id="tripz-message"
            className="max-h-36 min-h-10 w-full resize-none border-0 bg-transparent px-1 py-2 text-[13px] leading-5 text-[var(--text)] outline-none placeholder:text-[var(--text-muted)] disabled:opacity-60"
            rows={1}
            maxLength={12_000}
            value={text}
        ...
        <div className="flex min-h-4 items-start justify-between gap-4 px-1 text-[9px] leading-4 text-[var(--text-muted)]">
        ...
        <p id="tripz-composer-error" className={`m-0 min-h-0 text-[10px] leading-4 text-[var(--warning-text)] ${error ? "block" : "hidden"}`} role="alert">{error}</p>
      </div>
    </div>
  );
}
