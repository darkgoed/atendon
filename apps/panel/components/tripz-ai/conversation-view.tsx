...
function MessagesSkeleton() {
  return (
    <div className="mx-auto grid w-full max-w-[52rem] gap-5 py-4" role="status" aria-label="Carregando conversa">
...
function ProcessingCard({
  ...
  if (status === "failed") {
    return (
      <div className="mr-auto grid max-w-[36rem] grid-cols-[2.25rem_minmax(0,1fr)] items-start gap-3 border-l-2 border-[var(--warning)] py-3 pl-3 pr-4" role="alert">
        <span className="grid h-9 w-9 place-items-center border rounded-[var(--radius-md)] border-[var(--warning-border)] bg-[var(--warning-subtle)] text-[var(--warning-text)]"><WarningCircle size={17} weight="duotone" aria-hidden="true" /></span>
  ...
  return (
    <div className="mr-auto grid max-w-[32rem] grid-cols-[2.25rem_minmax(0,1fr)] items-center gap-3 border-l-2 border-[var(--primary)] py-3 pl-3 pr-4" role="status" aria-live="polite" aria-atomic="true">
      <span className="grid h-9 w-9 place-items-center border rounded-[var(--radius-md)] border-[var(--primary-border)] bg-[var(--primary-subtle)] text-[var(--primary)]">
...
function ConversationEmpty() {
  return (
    <div className="mx-auto grid min-h-full w-full max-w-[52rem] content-center px-2 py-6 text-left">
    ...
    <section className={styles.conversation} aria-label={`Conversa ${conversation.title}`}>
      ...
      <div ref={scrollRef} className={`${styles.scrollRegion} min-h-0 overflow-y-auto overscroll-contain px-4 py-5 sm:px-4`} aria-busy={loading}>
        {loading ? <MessagesSkeleton /> : error ? (
          <div className="mx-auto grid min-h-full w-full max-w-[36rem] place-items-center py-6 text-center" role="alert">
            <div>
          ...
          <div className="mx-auto grid w-full max-w-[52rem] gap-5 pb-3">
            ...
            {proposal && (proposal.status !== "collecting" || proposal.inconsistencies.length > 0) ? (
              <button type="button" className="mr-auto grid max-w-[38rem] grid-cols-[auto_minmax(0,1fr)] gap-3 border-y border-[var(--border)] bg-transparent px-1 py-4 text-left transition-[border-color,transform] hover:border-[var(--primary)] active:translate-y-px" onClick={onOpenReview}>
                <span className="grid h-10 w-10 place-items-center border rounded-[var(--radius-md)] border-[var(--primary-border)] bg-[var(--primary-subtle)] text-[var(--primary)]"><FileText size={18} weight="duotone" aria-hidden="true" /></span>
                <span className="grid gap-1">
