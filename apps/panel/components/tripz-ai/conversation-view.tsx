...
function ProcessingCard({
  ...
  if (status === "failed") {
    return (
      <div className="mr-auto grid max-w-[36rem] grid-cols-[2.25rem_minmax(0,1fr)] items-start gap-3 border-l-2 border-[var(--warn)] py-3 pl-3 pr-4" role="alert">
        <span className="grid h-9 w-9 place-items-center border border-[var(--warn-border)] bg-[var(--warn-bg)] text-[var(--warn)]"><WarningCircle size={17} weight="duotone" aria-hidden="true" /></span>
        <span className="grid gap-2">
          <span><strong className="block text-[11px] text-[var(--body)]">A análise foi interrompida</strong><span className="text-[10px] leading-4 text-[var(--muted)]">Seus dados continuam salvos. Tente processar o mesmo turno novamente.</span></span>
          {conversation.processingErrorCode ? <code className="text-[9px] text-[var(--faint-text)]">{conversation.processingErrorCode}</code> : null}
          {retryError ? <span className="text-[10px] text-[var(--warn)]">{retryError}</span> : null}
  ...
  return (
    <div className="mr-auto grid max-w-[32rem] grid-cols-[2.25rem_minmax(0,1fr)] items-center gap-3 border-l-2 border-[var(--accent)] py-3 pl-3 pr-4" role="status" aria-live="polite" aria-atomic="true">
      <span className="grid h-9 w-9 place-items-center border border-[var(--border-ai)] bg-[var(--accent-bg)] text-[var(--accent)]">
        <Robot size={17} weight="duotone" aria-hidden="true" />
      </span>
      <span className="grid gap-1">
        <strong className="text-[11px] font-semibold text-[var(--body)]">{tripzProcessingStatusLabel(status)}</strong>
        <span className="flex gap-1" aria-hidden="true">
          {[0, 1, 2].map((index) => <i key={index} className="h-1 w-8 animate-pulse bg-[var(--accent)] opacity-60" style={{ animationDelay: `${index * 140}ms` }} />)}
        </span>
      </span>
    </div>
  );
...
function ConversationEmpty() {
  return (
    <div className="mx-auto grid min-h-full w-full max-w-[52rem] content-center px-2 py-10 text-left">
      <div className="max-w-[34rem] border-l-2 border-[var(--accent)] pl-6 sm:pl-8">
        <span className="font-mono text-[9px] uppercase tracking-[.18em] text-[var(--accent)]">Comece pela informação mais fácil</span>
        ...
        <p className="mb-0 mt-4 max-w-[48ch] text-[13px] leading-6 text-[var(--muted)]">Envie uma frase, prints dos voos, imagens da hospedagem ou um PDF. A Tripz IA organiza os dados e pergunta apenas o próximo ponto necessário.</p>
        <div className="mt-7 grid gap-2 border-t border-[var(--border)] pt-4 text-[11px] text-[var(--faint-text)] sm:grid-cols-2">
      ...
      <header className="flex min-h-16 items-center justify-between gap-3 border-b border-[var(--border)] bg-[var(--topbar)] px-3 py-3 sm:px-5">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <span className="mt-1 flex items-center gap-2 text-[10px] text-[var(--faint-text)]">
              <i className={`h-1.5 w-1.5 rounded-full ${conversation.status === "pdf_generated" ? "bg-[var(--ok)]" : conversation.status === "collecting" ? "bg-[var(--faint)]" : "bg-[var(--accent)]"}`} aria-hidden="true" />
        ...
        {loading ? <MessagesSkeleton /> : error ? (
          <div className="mx-auto grid min-h-full w-full max-w-[36rem] place-items-center py-10 text-center" role="alert">
            <div>
              ...
              <p className="mb-0 mt-2 text-xs leading-5 text-[var(--muted)]">{error}</p>
          ...
          <div className="mx-auto grid w-full max-w-[52rem] gap-7 pb-3">
            ...
            {proposal && (proposal.status !== "collecting" || proposal.inconsistencies.length > 0) ? (
              <button type="button" className="mr-auto grid max-w-[38rem] grid-cols-[auto_minmax(0,1fr)] gap-3 border-y border-[var(--border)] bg-transparent px-1 py-4 text-left transition-[border-color,transform] hover:border-[var(--accent)] active:translate-y-px" onClick={onOpenReview}>
                <span className="grid h-10 w-10 place-items-center border border-[var(--border-ai)] bg-[var(--accent-bg)] text-[var(--accent)]"><FileText size={18} weight="duotone" aria-hidden="true" /></span>
                <span className="grid gap-1">
                  <strong className="text-xs text-[var(--text)]">Resumo da proposta atualizado</strong>
                  <span className="text-[10px] leading-4 text-[var(--faint-text)]">{proposal.missingInformation.length ? `${proposal.missingInformation.length} informação(ões) ainda pendente(s)` : "Dados reunidos para a revisão final"}</span>
                </span>
              </button>
            ) : null}
