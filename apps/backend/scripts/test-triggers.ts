...
// Test-only tenant fixtures. Install AFTER runMigrations; never on production
// or fresh-migration databases (fresh-migrations.integration.test.ts must keep
// seeing a single default stage).
export async function installTestOnlyTriggers(client: Pick<pg.Client, "query">): Promise<void> {
