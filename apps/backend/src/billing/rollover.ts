...
export async function previewRollover(client: PoolClient, tenantId: string): Promise<{ currentBalance: number; projectedRollover: number }> {
  const [b, p, settings] = await Promise.all([
    client.query(`SELECT COALESCE(sum(generated_amount-consumed_amount-expired_amount),0)::bigint AS balance FROM rollover_ledger WHERE tenant_id=$1 AND (expires_at IS NULL OR expires_at > now())`, [tenantId]),
    client.query(`SELECT s.status,p.rollover_enabled,p.rollover_rate_bps,p.rollover_max_percentage_bps,u.included_limit,u.included_usage FROM tenant_subscriptions s JOIN plans p ON p.id=s.plan_id JOIN usage_periods u ON u.tenant_id=s.tenant_id AND u.status='OPEN' WHERE s.tenant_id=$1 LIMIT 1`, [tenantId]),
    getBillingSettings(client)
  ]);
  const x = p.rows[0]; if (!x || !x.rollover_enabled || x.included_limit == null || INVALID_STATUSES.includes(x.status)) return { currentBalance: n(b.rows[0]?.balance), projectedRollover: 0 };
  ...
  return { currentBalance: n(b.rows[0]?.balance), projectedRollover: Math.min(Math.floor(unused*rate/10000), Math.floor(n(x.included_limit)*cap/10000)) };
}
...
export async function truncateRolloverToPlanCap(client: PoolClient, tenantId: string, newPlanId: string): Promise<{ removed: number }> {
  const q = await client.query(`SELECT COALESCE(pl.limit_value,0)::bigint AS limit, p.rollover_max_percentage_bps FROM plans p LEFT JOIN plan_limits pl ON pl.plan_id=p.id AND pl.limit_key='MAX_AI_INTERACTIONS' WHERE p.id=$1`, [newPlanId]);
  const cap = q.rows[0] ? Math.floor(n(q.rows[0].limit) * n(q.rows[0].rollover_max_percentage_bps ?? 0) / 10000) : 0;
  const r = await client.query(`WITH b AS (SELECT COALESCE(sum(generated_amount-consumed_amount-expired_amount),0)::bigint AS balance FROM rollover_ledger WHERE tenant_id=$1 AND (expires_at IS NULL OR expires_at > now())), x AS (SELECT GREATEST(0,balance-$2)::bigint removed FROM b), u AS (UPDATE rollover_ledger l SET expired_amount=l.expired_amount+LEAST(x.removed,GREATEST(0,l.generated_amount-l.consumed_amount-l.expired_amount)) FROM x WHERE l.tenant_id=$1 AND x.removed>0 RETURNING 1) SELECT (SELECT removed FROM x) AS removed`, [tenantId, cap]);
