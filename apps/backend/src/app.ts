...
import { evolutionContactUpdates, evolutionMessage, evolutionMessageStatusUpdates, evolutionPresenceUpdates, evolutionStickerMessage, parseEvolutionEvent } from "./modules/whatsapp/evolution-webhook.js";
import { SessionRepository } from "./modules/whatsapp/session-repository.js";
...
export function buildApp() {
  ...
  app.post("/webhooks/evolution", {
    ...
    app.log.info({ event: event.event, instance: event.instanceName, dataKeys: Object.keys(event.data) }, "Evolution webhook received");
    ...
    if (event.event === "MESSAGES_UPSERT") {
      ...
      if (sticker?.fromMe) {
        try {
          const media = await whatsapp.downloadMedia(identity.id, sticker.externalId);
          await new StickerRepository(db).importWhatsAppSticker({
          ...
          app.log.warn({ err: error, tenantId: identity.tenantId, sessionId: identity.id, externalId: sticker.externalId }, "Could not import WhatsApp sticker into AI library");
        }
      }
      const message = evolutionMessage(event.data, { tenantId: identity.tenantId, sessionId: identity.id });
      if (message) {
        await new AiFollowUpRepository(db).cancelForContact(
          message.tenantId,
        ...
        if (message.kind === "contact") {
          const hours = await loadBusinessHoursConfig(db, message.tenantId);
          const now = new Date();
          if (!isWithinBusinessHours(now, hours)) {
            ...
            await new MessageRepository(db).recordInboundAndLoadContext(message, { claim: false });
          }
        }
      ...
      app.log.info({ tenantId: identity.tenantId, sessionId: identity.id, count: updates.length }, "Evolution message statuses parsed");
      for (const update of updates) {
        ...
        if (update.failed && update.fromMe !== false && statusChanged) {
          app.log.warn({ tenantId: identity.tenantId, sessionId: identity.id, externalId: update.externalId }, "WhatsApp rejected outbound message (ack ERROR)");
      ...
      app.log.info({ rawState, state, allDataKeys: Object.keys(event.data) }, "CONNECTION_UPDATE parsed state");
      ...
      app.log.info({ sessionId: identity.id, status, hasOwner: Boolean(owner), reason }, "Session status updated");
      if (status === "connected") {
        ...
        void loadBusinessHoursConfig(db, identity.tenantId)
          .then((hours) => isWithinBusinessHours(new Date(), hours)
            ? whatsapp.goOnline(identity.id)
            : whatsapp.setPresence(identity.id, "unavailable", 1))
          .then(() => app.log.info({ sessionId: identity.id }, "Evolution presence refresh attempted"))
          .catch((err) => app.log.warn({ err, sessionId: identity.id }, "Evolution presence refresh failed"));
      }
    }
