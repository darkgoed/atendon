...
export type CommercialDashboardInput = {
...
function resolvePeriod(
  ...
  if (input.period === "week") {
    ...
    if (!input.start || !input.end) {
      throw Object.assign(new Error("Informe as datas inicial e final"), { statusCode: 400 });
    }
    if (input.end < input.start) {
      throw Object.assign(new Error("A data final deve ser igual ou posterior à inicial"), { statusCode: 400 });
    }
    const days = Math.round(
      (Date.parse(`${input.end}T00:00:00.000Z`) - Date.parse(`${input.start}T00:00:00.000Z`)) / 86_400_000
    );
    if (days > 365) {
      throw Object.assign(new Error("O período personalizado pode ter no máximo 366 dias"), { statusCode: 400 });
    }
  ...
  return {
    startKey,
    endKey,
    start: localDateTimeToUtc(startKey, "00:00", timezone),
    end: localDateTimeToUtc(endKey, "00:00", timezone),
    todayStart: localDateTimeToUtc(today, "00:00", timezone),
    todayEnd: localDateTimeToUtc(shiftDateKey(today, 1), "00:00", timezone)
  };
}
