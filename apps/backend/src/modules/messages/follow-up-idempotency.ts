    ...
    for (let i = 0; i < attempts; i += 1) {
      ...
      if (row.status === "sent" && row.external_message_id) {
        const message = await db.query<{ id: string }>(
          `SELECT id FROM messages WHERE conversation_id=$1 AND external_message_id=$2 ORDER BY created_at DESC LIMIT 1`,
          [input.conversationId, row.external_message_id]
        );
