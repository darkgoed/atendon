...
describe("webhook refunds and ordering", () => {
  it("aprovado", async () => {
  ...
  it("duplicado", async () => {
    // Identidade própria: os demais casos deste arquivo também entregam
    // eventos, e reaproveitar `pay-${suffix}` faria a PRIMEIRA entrega deste
    // teste já cair no portão de idempotência de outro caso.
    const eventId = `dup-${suffix}`;
    const before = await subscription();
    const first = await deliver(eventId, "approved");
    expect(first.statusCode).toBe(200);
    expect(first.json().status).toBe("processed");

    // A REENTREGA do mesmo evento é o objeto do teste.
    const second = await deliver(eventId, "approved");
    expect(second.statusCode).toBe(200);
    expect(second.json().status).toBe("duplicated");
    ...
    // O ponto que contagem de linhas não pegaria: o período NÃO pode ter avançado duas vezes.
    const after = await subscription();
    expect(after.current_period_end).toEqual(before.current_period_end);
    expect(after.status).toBe("ACTIVE");
  });

  it("provider não homologado é barrado na borda", async () => {
    ...
    // Um código desconhecido é barrado antes de qualquer consulta ao banco:
    // a rota pública só aceita códigos homologados.
