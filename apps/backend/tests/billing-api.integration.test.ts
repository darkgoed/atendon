...
describe("billing HTTP/Postgres authorization and integrity", () => {
  ...
  it("PATCH root billing settings rejeita tipo errado, faixa e chave desconhecida (400, não 500)", async () => { const cookie=await login(emails.root); expect((await app.inject({method:"PATCH",url:"/root/billing/settings",headers:{cookie},payload:{usd_brl_rate_micros:"abc"}})).statusCode).toBe(400); expect((await app.inject({method:"PATCH",url:"/root/billing/settings",headers:{cookie},payload:{campo_intruso:1}})).statusCode).toBe(400); expect((await app.inject({method:"PATCH",url:"/root/billing/settings",headers:{cookie},payload:{reservation_ttl_minutes:0}})).statusCode).toBe(400); expect((await app.inject({method:"PATCH",url:"/root/billing/settings",headers:{cookie},payload:{credit_min_cents:-5}})).statusCode).toBe(400); });
  it("GET root billing metrics rejeita start/end fora do formato de data (400) e aceita ISO de dia", async () => { const cookie=await login(emails.root); expect((await app.inject({url:"/root/billing/metrics?start=garbage",headers:{cookie}})).statusCode).toBe(400); expect((await app.inject({url:"/root/billing/metrics?end=também-garbage",headers:{cookie}})).statusCode).toBe(400); expect((await app.inject({url:`/root/billing/metrics?start=${new Date().toISOString().slice(0,10)}`,headers:{cookie}})).statusCode).toBe(200); });
