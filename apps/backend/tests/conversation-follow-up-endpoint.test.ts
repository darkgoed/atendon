...
import { enqueueFollowUpOnce, type FollowUpDb } from "../src/modules/messages/follow-up-idempotency.js";
...
describe("POST /conversations/:id/follow-up contract", () => {
  it("accepts the happy path immediately, without waiting for processing", async () => {
    ...
    const operation = vi.fn(() => new Promise<{ externalId: string; messageId: string }>(() => undefined));
    const result = await enqueueFollowUpOnce(db as unknown as FollowUpDb, { tenantId: "tenant-1", conversationId: "c", idempotencyKey: "happy-key-1" }, operation);
  ...
  it("returns one request id for concurrent retries and sends once", async () => {
    ...
    const [a, b] = await Promise.all([
      enqueueFollowUpOnce(db as unknown as FollowUpDb, { tenantId: "t", conversationId: "c", idempotencyKey: "same-key-1" }, operation),
      enqueueFollowUpOnce(db as unknown as FollowUpDb, { tenantId: "t", conversationId: "c", idempotencyKey: "same-key-1" }, operation)
    ]);
  ...
  it("keeps conflict and unavailable errors machine-readable", async () => {
    ...
    await expect(enqueueFollowUpOnce(db as unknown as FollowUpDb, { tenantId: "t", conversationId: "c", idempotencyKey: "conflict-1" }, vi.fn()))
  ...
  it("does not permit a conversation from another tenant", async () => {
    ...
    await expect(enqueueFollowUpOnce(db as unknown as FollowUpDb, { tenantId: "tenant-a", conversationId: "tenant-b-conversation", idempotencyKey: "tenant-1" }, vi.fn()))
...
describe("follow-up lock heartbeat", () => {
  ...
  it("logs heartbeat failure and clears interval on processor error", async () => {
    ...
    vi.doMock("../src/modules/messages/humanizer.js", async () => {
      const actual = await vi.importActual<typeof import("../src/modules/messages/humanizer.js")>("../src/modules/messages/humanizer.js");
    ...
    vi.spyOn(repository, "claimDue").mockResolvedValue({ conversationId: "c", tenantId: "t", sessionId: "s", contactPhone: "p", sequenceVersion: 1, history: [], model: "m", provider: "p", temperature: 1, maxTokens: 10 } as unknown as Awaited<ReturnType<InstanceType<typeof AiFollowUpRepository>["claimDue"]>>);
    vi.spyOn(repository, "releaseClaim").mockResolvedValue(undefined);
    let rejectAi!: (error: Error) => void;
    const ai = { complete: vi.fn(() => new Promise((_, reject) => { rejectAi = reject; })) } as unknown as ConstructorParameters<typeof AiFollowUpProcessor>[2];
    const error = vi.spyOn(console, "error").mockImplementation(() => undefined);
    // O 2º parâmetro é o gateway de mensagens ([1]); o 3º é o roteador de IA ([2]).
    const processor = new AiFollowUpProcessor(repository, {} as unknown as ConstructorParameters<typeof AiFollowUpProcessor>[1], ai);
