...
beforeAll(async () => {
  ...
  try {
    ...
    await c.query("INSERT INTO billing_providers(code,name,enabled,environment,status,credentials_encrypted) VALUES('mercadopago','Mercado Pago',true,$1,'DISCONNECTED',$2),('mercadopago','Mercado Pago',true,$3,'DISCONNECTED',$2)", ["sandbox", encryptCredentials({ clientId: "client", clientSecret: "secret", accessToken: "access", refreshToken: "refresh", redirectUri: "https://panel.test/root/saas/gateways" }, config.DATA_ENCRYPTION_KEY), "production"]);
